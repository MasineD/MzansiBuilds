...
const ProjectDetails = ({ project, showProjectDetails, onCancel, fetchProjects }) => {
  ...
  const fetchCollaborators = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/collaborators/${project.id}`);
  ...
  const addCollaborator = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/collaborator', {
        project_id: project.id,
  ...
  const removeCollaborator = async (collaboratorId, name) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/collaborator/${collaboratorId}`);
