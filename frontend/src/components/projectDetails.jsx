...
const ProjectDetails = ({ project, showProjectDetails, onCancel, fetchProjects }) => {
  ...
  // ========== NEW: Fetch collaborators ==========
  const fetchCollaborators = async () => {
    try {
      const response = await axios.get(`${API_URL}/api/collaborators/${project.id}`);
      setCollaborators(response.data.collaborators || []);
      setCollaboratorCount(response.data.collaborators?.length || 0);
    } catch (error) {
      console.error('Error fetching collaborators:', error);
    }
  };

  // ========== NEW: Add collaborator ==========
  const addCollaborator = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/collaborator`, {
      ...
      alert(error.response?.data?.msg || 'Error adding collaborator');
    } finally {
  ...
  const removeCollaborator = async (collaboratorId, name) => {
    const confirmation = window.confirm(`Are you sure you want to remove ${name} as a collaborator?`);
    if (!confirmation) return;
    
    try {
      await axios.delete(`${API_URL}/api/collaborator/${collaboratorId}`);
  ...
  const updateProjectStatus = async (newCompletedStatus) => {
    try {
      ...
      await axios.put(`${API_URL}/api/projects/${project.id}`, updatedFormData);
  ...
  const checkAndUpdateProjectStatus = async (updatedMilestones) => {
    if (!updatedMilestones || updatedMilestones.length === 0) return;
  ...
  const fetchMilestones = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_URL}/api/milestones/${project.id}`);
  ...
  useEffect(() => {
    if (project?.id) {
      fetchMilestones();
      fetchCollaborators(); // ========== NEW: Fetch collaborators ==========
    }
  }, [project?.id]);
  ...
  const addMilestone = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/milestone`, {
        project_id: project.id,
  ...
  const updateMilestoneStatus = async (milestoneId, currentStatus, description) => {
    try {
      ...
      await axios.put(`${API_URL}/api/milestone/${milestoneId}`, {
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                {collaborators.length > 0 && (
                  <div className="space-y-2 mb-3 max-h-40 overflow-y-auto">
                    {collaborators.map((collab) => (
                      <div key={collab.id} className="flex items-center justify-between gap-2 w-full px-4 py-2 border rounded-lg bg-gray-50">
                ...
                {!isEditing && collaboratorCount === 0 && (
                  <p className="text-sm text-gray-500 italic">No collaborators added yet</p>
                )}
                {!isEditing && collaboratorCount > 0 && (
                  <p className="text-xs text-gray-500">{collaboratorCount} collaborator{collaboratorCount !== 1 ? 's' : ''} on this project</p>
                )}
              </div>
